...
class LaptopActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            if (id !=0){
                ...
                if (bundle != null) {
                    txtTipe.setText(bundle.getString("MainActSeri"))
                }
                if (bundle != null) {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId){
            R.id.action_save -> {
                var Adapter = LaptopAdapter(this)

                var values = ContentValues()
                values.put("Merk", txtMerk.text.toString())
                values.put("Seri", txtTipe.text.toString())
                values.put("Harga", txtHarga.text.toString())

                if (id == 0){
                    val mID = Adapter.insert(values)
                    ...
                    val mID = Adapter.update(values, "Id=?", selectionArgs)
                    if (mID > 0){
